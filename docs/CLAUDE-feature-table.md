...
| 機能 | 活用スキル | 用途 |
|------|-----------|------|
| **Phase 93 Cross-Session Relay 内部実装 (独立セッション間 relay + cross-agent ハンドオフ)** | hooks, harness-work, codex, cursor | `A: 実装あり`。独立セッション間 relay を agmsg 非 install の内部実装で実現。`scripts/session-relay-watch.sh` (read, high-water mark / signal trap / pidfile race を agmsg watch.sh から移植) + `session-relay-send.sh` (write, jq -cn 安全 JSON) + `.claude/sessions/relay-signals.jsonl` (双方向 from→to addressing、harness-mem redaction に乗る)。`go/internal/hookhandler/session_relay_start.go` の `HandleSessionRelayStart` (monitor: SessionStart stdout directive で Monitor 起動) + `HandleRelayPoll` (turn: watch.sh --once exec)。`HARNESS_SESSION_RELAY=monitor\|turn\|both\|off` (default OFF)。即時 push は CC↔CC 限定、cross-agent は companion turn pull (codex/cursor-companion.sh が relay-notify.sh を source、構造的 redaction)。harness-loop と Monitor tool 二者択一。 |
| **Phase 92 Claude Code 2.1.153-2.1.162 + Codex 0.135-0.137 upstream refresh** | upstream-update, agent-view, guardrails | `A: 2 件 / C: 多数 / P: 2 件 / Reject (B: 0 件)`。`docs/upstream-update-snapshot-2026-06-04.md`。A: `claude agents --json` の `waitingFor` を `docs/agent-view-policy.md` に反映 (stuck 検知 diagnostic) / `codex plugin list --json` (低優先)。C: Opus 4.8 / SendMessage TMPDIR 修正 (CCH 未設定) / 2.1.160 shell-config gate (CCH は repo 内 `.claude/hooks` + settings deny、home shell は CC 本体、責務分離で差分なし) / Codex sandbox 強化。P: dynamic workflows (breezing 棲み分け) / auto mode プロバイダ拡大。Reject: app-server v2 (remote-control RPC、全面刷新でない) / TUI / VSCode 統合。 |
