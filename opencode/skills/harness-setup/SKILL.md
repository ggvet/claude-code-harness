...
- **`~/.claude/settings.json` の sandbox**: `network.allowedDomains` に `*.cursor.sh` を追加する。
  これにより cursor-agent を per-run の sandbox 無効化なしで実行できる。手順は
  `docs/sandbox-allowlist-recipe.md` の jq merge レシピに従う。CC 完全再起動後に有効化される。
