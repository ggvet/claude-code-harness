...
#### Reviewer-only mode (`--cursor --reviewer-only`)

Worker 実装は既完了（別系統 = claude / Codex で済んだ）、Reviewer のみ独立系統 (Composer) で回したい時の lean path:

1. 同上の 1 行 echo (Reviewer タスクとして)
2. `bash scripts/cursor-companion.sh task --mode ask --workspace <wt> "diff レビュー: <base_ref>..HEAD"`
   - `--mode ask` 必須（read-only、`--force` でも override 不可、cursor-adapter-candidate.md spike で実測済）
   - 書込なしのため `--write` は付けない
3. cursor 出力 (REQUEST_CHANGES / APPROVE 相当) を Lead が解釈
4. APPROVE なら Plans.md `cc:done [hash]` を Lead が更新
